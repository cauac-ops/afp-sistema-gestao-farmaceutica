...
if (!isset($_SESSION['id_func'])) { header('Location: ' . BASE_URL . '/public/login_afp.php'); exit; }

$_db  = new Database();
$_conn = $_db->connect();

$_r = $_conn->prepare("SELECT cargo, nome_func FROM funcionario WHERE id_func = :id AND ativo = 1");
...
if ($_row) {
    ...
    session_destroy(); header('Location: ' . BASE_URL . '/public/login_afp.php'); exit;
}

$current = basename($_SERVER['PHP_SELF']);
function isActive($page) { global $current; return $current == $page ? 'active' : ''; }

$ebq  = $_conn->query("SELECT COUNT(*) FROM produto WHERE estoque_atual <= estoque_minimo")->fetchColumn();
$ag_h = $_conn->query("SELECT COUNT(*) FROM agendamento WHERE DATE(data_agenda) = CURDATE() AND status = 'Agendado'")->fetchColumn();
$rv_h = $_conn->query("SELECT COUNT(*) FROM receita WHERE data_validade < CURDATE() AND status = 'Válida'")->fetchColumn();
...
<nav class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="<?= BASE_URL ?>/pages/dashboard.php"     class="nav-link-side <?= isActive('dashboard.php') ?>"><i class="bi bi-speedometer2"></i>Dashboard</a>
    <a href="<?= BASE_URL ?>/pages/vendas.php"        class="nav-link-side <?= isActive('vendas.php') ?>"><i class="bi bi-cart-check"></i>Nova Venda</a>
    <a href="<?= BASE_URL ?>/pages/historico_vendas.php" class="nav-link-side <?= isActive('historico_vendas.php') ?>"><i class="bi bi-receipt"></i>Histórico de Vendas</a>

    <div class="sidebar-section">Cadastros</div>
    <a href="<?= BASE_URL ?>/pages/produtos.php"      class="nav-link-side <?= isActive('produtos.php') ?>"><i class="bi bi-box-seam"></i>Produtos<?= $ebq > 0 ? "<span class='badge-side'>$ebq</span>" : '' ?></a>
    <a href="<?= BASE_URL ?>/pages/clientes.php"      class="nav-link-side <?= isActive('clientes.php') ?>"><i class="bi bi-people"></i>Clientes</a>
    <?php if (in_array($_SESSION['cargo'] ?? '', ['Gerente', 'Administrador', 'Farmacêutico'])): ?>
    <a href="<?= BASE_URL ?>/pages/funcionarios.php"  class="nav-link-side <?= isActive('funcionarios.php') ?>"><i class="bi bi-person-badge"></i>Funcionários</a>
    <?php endif; ?>

    <div class="sidebar-section">Clínico</div>
    <a href="<?= BASE_URL ?>/pages/agendamentos.php"  class="nav-link-side <?= isActive('agendamentos.php') ?>"><i class="bi bi-calendar-check"></i>Agendamentos<?= $ag_h > 0 ? "<span class='badge-side'>$ag_h</span>" : '' ?></a>
    <a href="<?= BASE_URL ?>/pages/receitas.php"      class="nav-link-side <?= isActive('receitas.php') ?>"><i class="bi bi-file-medical"></i>Receitas<?= $rv_h > 0 ? "<span class='badge-side text-warning' style='background:#b45309'>$rv_h</span>" : '' ?></a>

    <div class="sidebar-section">Análises</div>
    <?php if (in_array($_SESSION['cargo'] ?? '', ['Gerente', 'Administrador', 'Farmacêutico'])): ?>
    <a href="<?= BASE_URL ?>/pages/relatorios.php"    class="nav-link-side <?= isActive('relatorios.php') ?>"><i class="bi bi-graph-up-arrow"></i>Relatórios</a>
    <?php endif; ?>
    <a href="<?= BASE_URL ?>/pages/estoque.php"       class="nav-link-side <?= isActive('estoque.php') ?>"><i class="bi bi-clipboard2-pulse"></i>Estoque</a>
  </nav>
  <div class="sidebar-footer">
    <a href="<?= BASE_URL ?>/public/logout.php" class="btn-logout"><i class="bi bi-box-arrow-left"></i>Sair do Sistema</a>
  </div>
</nav>

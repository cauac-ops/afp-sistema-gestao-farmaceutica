...
if (isset($_GET['del']) && is_numeric($_GET['del']) && $_GET['del'] != $_SESSION['id_func']) {
    ...
    header('Location: ' . BASE_URL . '/pages/funcionarios.php?ok=Funcion%C3%A1rio+desativado'); exit;
}
...
if (isset($_GET['reativar']) && is_numeric($_GET['reativar']) && $_GET['reativar'] != $_SESSION['id_func']) {
    ...
    header('Location: ' . BASE_URL . '/pages/funcionarios.php?ok=Funcion%C3%A1rio+reativado'); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        header('Location: ' . BASE_URL . '/pages/funcionarios.php?ok=Funcion%C3%A1rio+atualizado'); exit;
    } else {
        if (!$senha) { header('Location: ' . BASE_URL . '/pages/funcionarios.php?erro=Informe+uma+senha'); exit; }
        ...
        header('Location: ' . BASE_URL . '/pages/funcionarios.php?ok=Funcion%C3%A1rio+cadastrado'); exit;
    }
}
